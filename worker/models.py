...
class RecentResult(Camel):
    ...
    def _kst(cls, v: datetime | None) -> datetime | None:
        ...
        **`Message.sentAt` 과 같은 이유인데 여기는 터지는 방식이 다르다.** 이 값은
        `ctx.now`·세그먼트 시작 시각(둘 다 타임존 인식)과 **비교·빼기**를 한다
        (`Context.last_result_at` 의 사용처). 규격서 예시가 `"2026-08-14T21:00:00"` 처럼
        오프셋 없이 오므로, 정규화하지 않으면 그 연산이 `TypeError` 로 터지고
        요청 전체가 `MODEL_ERROR` 가 된다.
        """
