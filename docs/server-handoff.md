...
빼는 것(중복 방지)과, **최근 30분 안에 영상을 냈으면 유튜브를 아예 안 돌리는 것**입니다.
후자는 YouTube 무료 쿼터(하루 약 94회 발동)를 지키려고 넣었습니다 — 메시지마다 워커를
부르는 구조라 억제가 없으면 대화 한 번에 하루치가 마릅니다.
...
코스에서 뺍니다. `createdAt` 은 유튜브 쿨다운(2장)에도 쓰입니다.
