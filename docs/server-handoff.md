...
>
> 이제 단건 반영 시간(실측 10.4~14.4초)은 **순수 워커 처리 시간**입니다. 방 1의
> 실제 대화 이력에서는 잡담에도 데이트 계획 같은 무거운 가지가 게이트를 통과해
> (결과 미발동이어도 LLM·카카오 호출은 돌아서) 로컬 잡담 픽스처와 같은 9~12초가
> 나옵니다. 다음 지렛대는 워커 쪽이고 백엔드 액션은 없습니다.
