...
## 17단계 — 빠른 모델(gpt-4.1-nano) 시도 ❌ 되돌림

"4초 응답"을 목표로 채점·판정 단계만 `gpt-4.1-nano` 로 내려봤다. **48% 빨랐지만 판정이
무너져서 전부 되돌렸다.** 코드는 남기지 않았다 — 다음에 같은 시도를 반복하지 않도록
결과만 기록한다.

| 단계 | 결과 |
| --- | --- |
| 말투 판정 | ❌ **`case8_banter` 를 2/2 로 갈등 판정.** 장난에 교정 카드가 뜬다 — `CLAUDE.md` 가 "최대 리스크"로 못박은 실패다 |
| | ❌ `case13_mukbang` 도 오발동. 그러면 `SUPPRESS_YOUTUBE_WHEN_TONE` 때문에 화제 갈래가 통째로 죽는다 |
| 실 상태 채점 | ❌ 점수를 낮게 매겨 `MIN_SCORE=3` 을 못 넘긴다. `case9`·`case13` 이 `ENGAGED` → `STABLE`. 화면이 전부 "평온해요"가 된다 |
| 대화 분절 | ❌ 14개 중 2개에서 **경계를 통째로 놓친다** (`case2` 4개→1개, `case5` 2개→1개). 분절이 무너지면 두 시간 전 화제가 지금 요청에 딸려 온다 |

처음엔 분절만 6/6 동일해서 살릴까 했는데, 픽스처를 14개로 늘리니 깨졌다.
**표본 6개로 판단할 뻔했다.**

### 그래서 4초는 개입이 없을 때만 나온다

병렬화는 16단계에서 끝났고, 남은 건 전부 직렬이다. 시간은 **LLM 왕복 횟수**가 정한다.

    개입 없음    분절 → 상태                    3.9s  ← 목표 달성
    말투 교정    분절 → 판정 → 생성             5.5~6.2s
    유튜브       분절 → 분류 → 검색 → 선정      7.0s
    데이트 코스  분절 → 계획 → 카카오 → 문구     8.1s

중간에 외부 호출이 끼어 있어 합칠 수 없다 — 카카오 결과를 봐야 문구를 쓰고, 검색
결과를 봐야 영상을 고른다. 그게 환각 방지 설계의 핵심이다.

**남은 방법은 왕복을 없애는 것뿐이고 그건 기능 변경이다.** 서버 타임아웃이 15초든
30초든 최악이 8.1초라 여유가 있으므로, 지금은 하지 않는다.
