...
- [ ] 후보 기능 병렬 실행 — 지금은 순차라 3개가 다 발동하면 느리다.
      **타임아웃 기준이 정해진 뒤에 한다** (`docs/contract-review.md` 4번)
- [ ] 분절 채점의 `note` 출력 줄이기 — 발화마다 근거 문장을 써서 분절 호출이 3.8초다
      (배치 방식 1.4초). 20자 제한 → 회색지대만 → 제거 순으로 (`segmentation-v3.md` 9장)
...
- [ ] 서버 담당자: 요청에 `recentResults` · `speakerProfiles` 추가 요청 (둘 다 🔴)
- [ ] 서버 담당자: `category` enum 8종 확정 (🔴 — 확정 전에는 응답 검증에서 반려될 수 있다)
- [ ] 서버 담당자: 1회 요청 메시지 개수(20~30) + 타임아웃 기준(30초 제안)
